        ...
        <table class="table-auto w-full border-collapse border border-gray-300">
            ...
            @foreach($agents as $agent)
                <tr class="hover:bg-gray-100">
                    ...
                    <td class="border border-gray-300 p-2">{{ str_repeat('•', strlen($agent->password)) }}</td>
                    ...
                    <td class="border border-gray-300 p-2 flex space-x-2">
                        ...
                        <button onclick="toggleDrawer('editDrawer', {{ $agent->id }})" title="Edit Agent">
                        ...
                        <form action="{{ route('agents.destroy', $agent->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this item?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-transparent border-0 p-0" title="Delete Agent">
                                <svg width="34px" height="34px" viewBox="-204.8 -204.8 1433.60 1433.60" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000" stroke="#000000" stroke-width="0.01024">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                                        <path d="M905.92 237.76a32 32 0 0 0-52.48 36.48A416 416 0 1 1 96 512a418.56 418.56 0 0 1 297.28-398.72 32 32 0 1 0-18.24-61.44A480 480 0 1 0 992 512a477.12 477.12 0 0 0-86.08-274.24z" fill="#ff0000"></path>
                                        <path d="M630.72 113.28A413.76 413.76 0 0 1 768 185.28a32 32 0 0 0 39.68-50.24 476.8 476.8 0 0 0-160-83.2 32 32 0 0 0-18.24 61.44zM489.28 86.72a36.8 36.8 0 0 0 10.56 6.72 30.08 30.08 0 0 0 24.32 0 37.12 37.12 0 0 0 10.56-6.72A32 32 0 0 0 544 64a33.6 33.6 0 0 0-9.28-22.72A32 32 0 0 0 505.6 32a20.8 20.8 0 0 0-5.76 1.92 23.68 23.68 0 0 0-5.76 2.88l-4.8 3.84a32 32 0 0 0-6.72 10.56A32 32 0 0 0 480 64a32 32 0 0 0 2.56 12.16 37.12 37.12 0 0 0 6.72 10.56zM726.72 297.28a32 32 0 0 0-45.12 0l-169.6 169.6-169.28-169.6A32 32 0 0 0 297.6 342.4l169.28 169.6-169.6 169.28a32 32 0 1 0 45.12 45.12l169.6-169.28 169.28 169.28a32 32 0 0 0 45.12-45.12L557.12 512l169.28-169.28a32 32 0 0 0 0.32-45.44z" fill="#ff0000"></path>
                                    </g>
                                </svg>
                            </button>
                        </form>
    ...
    <script>
        function toggleDrawer(drawerId, agentId = null) {
            ...
            if (drawer.classList.contains('open')) {
                ...
                // Clear the form fields when the drawer is closed
                if (drawerId === 'createDrawer') {
                    document.getElementById('createFormContainer').innerHTML = '';
                } else if (drawerId === 'editDrawer') {
                ...
                if (drawerId === 'editDrawer' && agentId) {
                    fetch(`/agent/${agentId}/edit`)  // Adjusted the route path
                        .then(response => response.text())
                        .then(data => {
                            document.getElementById('editFormContainer').innerHTML = data;
                        });
                } else if (drawerId === 'createDrawer') {
                    fetch('/agent/create')  // Ensure this route exists and returns the form
                        .then(response => response.text())
                        .then(data => {
        ...
        // Attach form validation to the 'submit' event of the form inside the drawer
            ...
            if (createForm) {
                createForm.addEventListener('submit', function (e) {
                    if (!validateForm(createForm)) {
                        ...
                        toggleDrawer('createDrawer');  // Only close drawer if validation passes
                    }
                });
            }
            ...
            if (editForm) {
                editForm.addEventListener('submit', function (e) {
                    if (!validateForm(editForm)) {
                        ...
                        toggleDrawer('editDrawer');  // Only close drawer if validation passes
                    }
                });
            }
        });
        ...
        function validateForm(form) {
            ...
            // Example validation: Check if fields are empty (you can add more rules here)
            const requiredFields = form.querySelectorAll('[required]');
            requiredFields.forEach(field => {
                if (!field.value.trim()) {
                    isValid = false;
                    field.classList.add('border-red-500');  // Adding a red border to show error
                } else {
                    field.classList.remove('border-red-500');  // Remove error class if valid
                }
            });
