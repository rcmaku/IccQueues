...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt(['email' => $request->email, 'password' => $request->password], $request->filled('remember-me'))) {
            ...
            // Fetch or create the 'unavailable' status
            $unavailableStatus = AgentStatus::firstOrCreate(['name' => 'unavailable']);

            // Update the user's status to "unavailable"
            $user->agentStatus()->associate($unavailableStatus);
            $user->save();

            // Log the status change in the AgentStatusHistory
            AgentStatusHistory::create([
                'user_id' => $user->id,
                'agent_status_id' => $unavailableStatus->id,
                'changed_at' => now(),
            ]);
